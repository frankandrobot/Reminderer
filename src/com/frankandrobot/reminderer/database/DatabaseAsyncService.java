...
public class DatabaseAsyncService extends IntentService {
	...
	protected void onDeleteComplete(int token, Object cookie, Integer result) {
	}

	protected void onUpdateComplete(int token, Object cookie, Integer result) {
     ...
     * 
     * Note: start the service to actually insert the values in the db.
     * 
     * postOp will be called after the insert.
     * 
     * handler will be called last.
     * 
     * @param context
     * @param handler
     * @param values
	...
	OperationInfo info = new OperationInfo(Operation.EVENT_ARG_INSERT,
		context.getContentResolver(), DbColumns.CONTENT_URI, handler,
		postOp);
	info.values = values;
